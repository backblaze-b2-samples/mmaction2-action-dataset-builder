...
| No ML/media SDKs (mmaction/torch/torchvision/scenedetect) outside repo/ | `tests/test_structure.py::test_ml_sdks_only_in_repo` |
...
The base API (`requirements.txt`) boots and tests run **without** the ML stack.
To actually run a build's labeling step, install the pinned ML deps:
`cd services/api && source .venv/bin/activate && pip install -r requirements-ml.txt`
(if the `mmcv` wheel fails: `pip install -U openmim && mim install "mmcv>=2.1,<2.2"`).
Keep `mmaction`/`torch`/`torchvision`/`scenedetect` imports inside `app/repo/` only.
