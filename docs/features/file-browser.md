...
## Purpose
List, preview, download, and delete **any** object stored in the Backblaze B2 bucket — the full-bucket browser (raw footage + all build artifacts), distinct from the build-scoped [Dataset Explorer](dataset-explorer.md).
...
- Client route-construction tests: `pnpm --filter @mmaction2-action-dataset-builder/web test`
- Full verify command: `pnpm lint && pnpm --filter @mmaction2-action-dataset-builder/web test && pnpm build && pnpm lint:api && pnpm test:api && pnpm check:structure`
