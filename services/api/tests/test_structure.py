...
# Heavy ML / media SDKs that must be confined to app/repo/ (mmaction_engine.py,
# video_tools.py), mirroring the boto3-only-in-repo invariant. cv2/decord/
# imageio are media-decode deps; torch is an mmaction transitive dep.
_ML_PACKAGES = (
    "mmaction",
    "mmcv",
    "mmengine",
...
def test_ml_sdks_only_in_repo():
    """Verify mmaction/torch/scenedetect/ffmpeg are only imported in app/repo/."""
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
            continue
